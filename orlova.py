...
class Ui_MainWindow(object):
    ...
    def main_guess(self, choice):
        if self.timer_seconds == 0:
            ...
            error = QMessageBox()
            error.setWindowTitle("Куда мы лезем боже")
            error.setText("Жди пока кнопка закончит свои действия")
            error.setIcon(QMessageBox.Warning)
            error.setStandardButtons(QMessageBox.Ok|QMessageBox.Close)
            error.setDefaultButton(QMessageBox.Ok)
            error.setInformativeText("Ну как бы программа как и человек не может смотреть в две стороны одновременно")
            error.setDetailedText("Тут либо теневых клонов кастуй либо жди свои пять секунд и хватит тыкать по кнопкам да")
            error.addButton("Убить прогу?", QMessageBox.RejectRole)
            bts = error.buttons()
            for button in bts:
                if button.text() == 'Убить прогу?':
                    error.setDefaultButton(button)
            error.buttonClicked.connect(self.warning_action)
            # error.setTextFormat()
            # error.setStyleSheet("font: 16pt;\n"
            #                         "color: rgb(265, 190, 255);\n"
            #                         )
            error.exec_()

    def warning_action(self, btn):
        if btn.text() == "OK":
            print('ok')
        elif btn.text() == "Close":
            error2 = QMessageBox()
            error2.setWindowTitle("Куда мы лезем боже")
            error2.setText("ЛОВУШКА АХАХАХАХ")
            error2.setIcon(QMessageBox.Warning)
            error2.setInformativeText("ДЖОКУШКЕР ")
            error2.setDetailedText(
                "ЛОШПЕДУС")
            error2.addButton("Убить прогу?", QMessageBox.RejectRole)
            error2.setStandardButtons(QMessageBox.Close)
            error2.buttonClicked.connect(self.warning_action)
            bts = error2.buttons()
            for button in bts:
                if button.text() == 'Убить прогу?':
                    error2.setDefaultButton(button)
            error2.exec_()
        elif btn.text() == "Убить прогу?":
            # sys.exit()
            error2 = QMessageBox()
            error2.setWindowTitle("Куда мы лезем боже")
            error2.setText("ЛОВУШКА АХАХАХАХ")
    ...
    def generate_string(self, choice):
        if choice == 1:
            ...
            self.timer_seconds += 1
        elif choice == 4 and self.label_month.text() != "Месяц" and self.label_day.text() != "Число" and self.label_year.text() != "Год" and not self.random_done:
            if self.timer_seconds == 0:
